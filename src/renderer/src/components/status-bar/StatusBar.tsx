...
function CodexSwitcherMenu({
  ...
  const handleSelectAccount = useCallback(
    async (accountId: string | null, target: CodexStatusRuntimeTarget): Promise<void> => {
    ...
    [accountState, fetchSettings, isSwitching, reauthenticatingAccountId, recordFeatureInteraction]
  )

  const autoFailoverTarget = useMemo(
    () => normalizeCodexStatusRuntimeTarget(accountState, toCodexStatusRuntimeTarget(codexTarget)),
    [accountState, codexTarget]
  )
  const autoFailoverActiveAccountId = getCodexStatusActiveId(accountState, autoFailoverTarget)
  const autoFailoverTargetKey = getCodexStatusRuntimeKey(autoFailoverTarget)

  const autoFailoverAccountId = chooseCodexFailoverAccount({
    activeAccountId: autoFailoverActiveAccountId,
  ...
  useEffect(() => {
    ...
    const signature = `${autoFailoverTargetKey}:${autoFailoverActiveAccountId ?? 'system'}:${codex.updatedAt}:${codex.status}:${codex.error ?? ''}:${autoFailoverAccountId}`
    if (
    ...
    autoFailoverKeyRef.current = signature
    void handleSelectAccount(autoFailoverAccountId, autoFailoverTarget)
  }, [
    autoFailoverActiveAccountId,
    autoFailoverAccountId,
    autoFailoverTarget,
    autoFailoverTargetKey,
    codex.error,
    codex.status,
    codex.updatedAt,
    handleSelectAccount,
