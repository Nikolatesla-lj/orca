...
function cancelDeferredFrame(frame: number): void {
...
export function connectPanePty(
  ...
  let connectFrameFallbackTimer: ReturnType<typeof setTimeout> | null = null
  ...
  const onDataDisposable = pane.terminal.onData((data) => {
    ...
    // xterm can also emit ESC-prefixed protocol replies while parsing terminal
    // output; those are not human interaction and must not self-dismiss a BEL.
  ...
  // Defer PTY spawn/attach so FitAddon has time to calculate dimensions from
  // the laid-out container. Some relaunch paths can pause rAF before the first
  // visible paint; the short timer fallback prevents an otherwise mounted
  // active terminal from staying permanently disconnected.
  const runDeferredConnect = (): void => {
    if (connectFrame !== null) {
      cancelDeferredFrame(connectFrame)
      connectFrame = null
    }
    if (connectFrameFallbackTimer !== null) {
      clearTimeout(connectFrameFallbackTimer)
      connectFrameFallbackTimer = null
    }
    if (disposed) {
      return
    }
  ...
  connectFrameFallbackTimer = setTimeout(runDeferredConnect, 100)
  if (typeof requestAnimationFrame === 'function') {
    ...
    connectFrame = frameRanSynchronously ? null : nextFrame
  }
  ...
  return {
    dispose() {
      ...
      if (pendingGeometryReportRaf !== null) {
        cancelDeferredFrame(pendingGeometryReportRaf)
        pendingGeometryReportRaf = null
      }
