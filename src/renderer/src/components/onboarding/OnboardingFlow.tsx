                ...
                <Tooltip key={step.id}>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      aria-label={`Go to onboarding step ${step.stepNumber}: ${stepTooltipLabels[step.id]}`}
          ...
          <div className="flex items-center gap-2">
            ...
            {currentStep.id !== 'repo' && (
              ...
              >
                ...
                <kbd
                  aria-hidden="true"
                  className="ml-1 inline-flex items-center gap-0.5 rounded border border-primary-foreground/20 px-1.5 py-0.5 text-[10px] font-medium leading-none text-current/80"
                >
                  <span aria-hidden="true">{isMac ? '⌘' : 'Ctrl'}</span>
                  <span className="sr-only">{isMac ? '⌘↵' : 'Ctrl+Enter'}</span>
