...
export type ScryerEditSessionController = {
  ...
  completeAgentEditSession(
    input: CompleteAgentEditSessionInput
...
function foldInputsFor(changes: PendingChange[]): ScryerPlanFoldInput[] {
...
export function evaluateCompletionGate(input: EvaluateCompletionGateInput): CompletionGateResult {
  ...
  const nextAction: CompletionGateNextAction =
    leaseGateBlocker
      ? 'blocked_by_lease'
...
export function createScryerEditSessionController(
  ...
  const controller: ScryerEditSessionController = {
    ...
    async readEditSession(input) {
      return {
        projectPath: input.projectPath,
        activeLease: toLeaseStatus(await options.leaseStore.read({ projectPath: input.projectPath }))
      }
    }
  }
