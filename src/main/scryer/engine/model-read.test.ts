...
describe('scryer.model.read', () => {
  it('creates and reads an empty Scryer 0.3 model on the plan layer', async () => {
    ...
    const raw = JSON.parse(await readFile(join(projectPath, '.scryer', 'model.scry'), 'utf8'))
    expect(raw.version).toBe('0.3')
  })
