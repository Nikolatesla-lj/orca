...
export async function getTerminalContent(page: Page, charLimit = 4000): Promise<string> {
  ...
  return page.evaluate(
    ({ tabId, charLimit }) => {
      ...
      const readPaneText = (pane: TerminalPaneLike): string => {
        const serialized = pane.serializeAddon?.serialize?.() ?? ''
        if (serialized.length > 0) {
          return serialized
        }

        // In Electron headless runs with the DOM renderer, daemon-restored
        // terminal text can be visible while the SerializeAddon still returns
        // an empty string. Read xterm's active buffer as the fallback so E2E
        // assertions check the same content a user can see on screen.
      ...
      if (!activePane) {
        ...
        const text = readPaneText(panes[0])
        return text.slice(-charLimit)
      }

      const text = readPaneText(activePane)
      return text.slice(-charLimit)
    },
    { tabId, charLimit }
  )
